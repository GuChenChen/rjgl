...
public class Oauth2AccessDecisionManager implements AccessDecisionManager {
    ...
    public void decide(Authentication authentication, Object resource, Collection<ConfigAttribute> configAttributes) throws AccessDeniedException, InsufficientAuthenticationException {
        FilterInvocation resource1 = (FilterInvocation) resource;
        if(HttpMethod.OPTIONS.name().equalsIgnoreCase(resource1.getRequest().getMethod())){
            return;
        }
        String requestUrl = resource1.getRequestUrl();
        log.info("[决策管理器]:开始判断请求 {} 需要的权限", requestUrl);
        if (configAttributes == null || configAttributes.isEmpty()) {
            log.info("[决策管理器]:请求 {} 无需权限", requestUrl);
            return;
        }

        log.info("[决策管理器]:请求 {} 需要的权限 - {}", requestUrl, configAttributes);
        ...
        log.info("[决策管理器]:用户 {} 没有访问资源 {} 的权限!", authentication.getName(), requestUrl);
